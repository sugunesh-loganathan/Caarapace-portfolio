...
void main(){
  ...
  // Sample the drifting fbm field and average it into a single smoke
  // "density" value in roughly [0,1].
  float d=fbm(uv+vec2(0,T*.015)+n);
  d+=fbm(uv*1.003+vec2(0,T*.015)+n+.003);
  d+=fbm(uv*1.006+vec2(0,T*.015)+n+.006);
  d/=3.;
  d=clamp(d,0.,1.);

  // Bright, clean smoke: a light base ramps through a vivid crimson
  // (u_color) in the mid-densities up to sharp white at the peaks — no grey
  // muddiness, just bright crimson-and-white plumes.
  vec3 base=vec3(1.0);
  vec3 col=mix(base, u_color, smoothstep(0.15,0.6,d));
  col=mix(col, vec3(1.0), smoothstep(0.55,1.0,d));

  // Fade in from the light base on load.
